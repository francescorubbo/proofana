...
void EventBuilder_jetmet2012pileupcustom::CopyMoreClusters(){
  ...
  ems.push_back("cl_E_EMB1");
  ems.push_back("cl_E_EMB2");
  ems.push_back("cl_E_EMB3");
  ems.push_back("cl_E_PreSamplerE");
  ems.push_back("cl_E_EME1");
  ems.push_back("cl_E_EME2");
  ems.push_back("cl_E_EME3");
  vector<string> hads;
  hads.push_back("cl_E_HEC0");
  hads.push_back("cl_E_HEC1");
  hads.push_back("cl_E_HEC2");
  hads.push_back("cl_E_HEC3");
  hads.push_back("cl_E_TileBar0");
  hads.push_back("cl_E_TileBar1");
  hads.push_back("cl_E_TileBar2");
  hads.push_back("cl_E_TileGap1");
  hads.push_back("cl_E_TileGap2");
  hads.push_back("cl_E_TileGap3");
  hads.push_back("cl_E_TileExt0");
  hads.push_back("cl_E_TileExt1");
  hads.push_back("cl_E_TileExt2");
  hads.push_back("cl_E_FCAL0");
  hads.push_back("cl_E_FCAL1");
  hads.push_back("cl_E_FCAL2");

  for(int iCl = 0; iCl<fEvt->clusters(cltype);++iCl){

    float e_em = 0.0;
    for(vector<string>::iterator it = ems.begin(); it != ems.end(); ++it)
      e_em += Get<vector<float> >(*it).at(iCl);       
    float e_had = 0.0;
    for(vector<string>::iterator it = hads.begin(); it != hads.end(); ++it)
      e_had += Get<vector<float> >(*it).at(iCl);       
    
    fEvt->cluster(iCl,cltype).Set("fem", e_em/(e_had+e_em));
    fEvt->cluster(iCl,cltype).Set("centerlambda", Get<vector<float> >("cl_centerlambda").at(iCl));
  }//clusters
}
