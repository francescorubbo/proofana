...
#define Analysis_voronoi_cvf_writeout_cxx

#include "Analysis_voronoi_cvf_writeout.h"
#include "AnaConfig.h"
#include <TROOT.h>
#include <TH1F.h>
#include <TH2F.h>
...
///=========================================
 void Analysis_voronoi_cvf_writeout::WorkerBegin()
 {
  if (Debug()) cout << "Analysis_voronoi_cvf_writeout: DEBUG In WorkerBegin() " << endl;
  ...
  if (Debug()) cout << "Analysis_voronoi_cvf_writeout: DEBUG Finish WorkerBegin()" << endl;  
...
///=========================================  
  int total_LCTopo_jets=0;
  int total_spread_jets=0;
  int total_truth_jets=0;
bool Analysis_voronoi_cvf_writeout::ProcessEvent()
{

  if (Debug()) cout << "Analysis_voronoi_cvf_writeout: DEBUG In ProcessEvent(): RunNumber = " << RunNumber() 
...
setMinDR("AntiKt4j0_CVF");
setMinDR("AntiKt4Voronoi_CVFx_SpreadPT");
...
MomKey voronoi_file="event_files/jet_centers_Voronoi_CVFcutx_"+event+".txt";
MomKey spread_file="event_files/jet_centers_spread_Voronoi_CVFcutx_"+event+".txt";
MomKey true_file="event_files/jet_centers_true_"+event+".txt";
ofstream fout(voronoi_file);
ofstream gout(spread_file);
ofstream hout(true_file);

for(int jindex=0; jindex<jets("AntiKt4j0_CVF"); jindex++){
Particle  *myjet         = &(jet(jindex,"AntiKt4j0_CVF"));
float jetpt = myjet->p.Pt();
float jeteta = myjet->p.Eta();
float jetphi = myjet->p.Phi();
bool truematch = myjet->Bool("isHSJet");
float jetmindr = myjet->Float("minDR");
fout << jeteta << "\t" << jetphi << "\t" << jetpt << "\t" << truematch << endl;

if(myjet->Bool("isHSJet")){
Particle *tjet = (Particle*) myjet->Obj("AntiKt4Truth_match");
float tjetpt = tjet->p.Pt();
float tjeteta = tjet->p.Eta();
float tjetphi = tjet->p.Phi();
float tjetmindr = tjet->Float("minDR");
if(20 < tjetpt && tjetpt < 30 && tjeteta>-1 && tjeteta<1 && tjetmindr>0.8){ total_LCTopo_jets++; LCTopo_jets++;}
}
}

for(int jindex=0; jindex<jets("AntiKt4Voronoi_CVFx_SpreadPT"); jindex++){
Particle  *myjet         = &(jet(jindex,"AntiKt4Voronoi_CVFx_SpreadPT"));
float jetpt = myjet->p.Pt();
float jeteta = myjet->p.Eta();
float jetphi = myjet->p.Phi();
bool truematch = myjet->Bool("isHSJet");
float jetmindr = myjet->Float("minDR");
gout << jeteta << "\t" << jetphi << "\t" << jetpt << "\t" << truematch <<endl;
...
float tjetphi = tjet->p.Phi();
float tjetmindr = tjet->Float("minDR");
if(20 < tjetpt && tjetpt < 30 && tjeteta>-1 && tjeteta<1 && tjetmindr>0.8){ total_spread_jets++; spread_jets++;}
}
}

for(int jindex=0; jindex<jets("AntiKt4Truth"); jindex++){
Particle  *myjet         = &(jet(jindex,"AntiKt4Truth"));
float tjetpt = myjet->p.Pt();
float tjeteta = myjet->p.Eta();
float tjetphi = myjet->p.Phi();
float tjetmindr = myjet->Float("minDR");
hout << tjeteta << "\t" << tjetphi << "\t" << tjetpt << endl;
if(20 < tjetpt && tjetpt < 30 && tjeteta>-1 && tjeteta<1 && tjetmindr>0.8){ total_truth_jets++; truth_jets++;}
}
...
//cout << LCTopo_jets << ";" << spread_jets << ";" << truth_jets << ";" << EventNumber() << endl;
//if(LCTopo_jets>spread_jets) cout << LCTopo_jets << ";" << spread_jets << ";" << truth_jets << ";" << EventNumber() << endl;
cout << total_LCTopo_jets << ";" << total_spread_jets << ";" << total_truth_jets << ";" << EventNumber() << endl;
...
///=========================================
void Analysis_voronoi_cvf_writeout::WorkerTerminate()
{

}

void Analysis_voronoi_cvf_writeout::setMinDR(MomKey key){
...
void Analysis_voronoi_cvf_writeout::SetCVF(MomKey key){
...
void Analysis_voronoi_cvf_writeout::MakeSpreadVoronoiClusters(float spreadr,MomKey key){
...
MomKey event=TString::Format("%i",EventNumber());
MomKey voronoi_file="event_files/cluster_centers_"+key+"_"+event+".txt";
MomKey sub_file="event_files/cluster_centers_sub_"+key+"_"+event+".txt";
MomKey spread_file="event_files/cluster_centers_spread_"+key+"_"+event+".txt";
ofstream fout(voronoi_file);
...
void Analysis_voronoi_cvf_writeout::selectClusters(MomKey key,int nsigma,bool doCVF,float threshold)
{
...
void Analysis_voronoi_cvf_writeout::selectLCTopoCVFClusters()
{
MomKey key="LCTopo";
MomKey clusterskey="clusters"+key;
clusterskey+="_CVFcut";
...
void Analysis_voronoi_cvf_writeout::selectVoronoiCVFClusters(float threshold){
MomKey key="Voronoi";
MomKey clusterskey="clusters"+key;
clusterskey+="_CVFcut";
if(threshold>0) clusterskey+=TString::Format("%i",(int)threshold);
...
MomKey Analysis_voronoi_cvf_writeout::MakeVoronoiClusters(const fastjet::JetAlgorithm algo, const double jetR, const MomKey constType){
